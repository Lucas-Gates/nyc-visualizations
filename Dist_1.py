...
#load data
df = pd.read_csv("data/cleaned_motor_vehicle_crashes.csv")
df = df.dropna(subset=["HOUR", "SEVERITY_LABEL"])
...
for severity in plot_order:
    subset = df[df["SEVERITY_LABEL"] == severity]
...
#save and show
plt.tight_layout()
plt.savefig("fig_distribution_danger_window.png")
plt.show()